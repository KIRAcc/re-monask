self.addEventListener('message', function(e) {
  ...
  for(i = 0; i<responses.length; i++) {
    ...
    html += '<p id="res_' +(i+1)+ '" style="margin-left:10px;">'; // レスのヘッダ（ブロック要素）
    html += '<span class="rh"><a href="#res_' +(i+1)+ '">' +(i+1)+ '</a></span> ：'; // レス番
    ...
    html += '<a class="send" href="javascript:void(0);" onclick="BBS.askmona.showSendWin(' + thread.t_id + ', ' + (i+1) + ')" title="' + escapeHtml(tipHtml) + '">←送る</a>'; //
