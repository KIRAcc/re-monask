self.addEventListener('message', function(e) {
  ...
  var isAnchor = ("ankaStr" in e.data);
  if (isAnchor) {
    var ankaStr = e.data.ankaStr
    ,   ankaArr = ankaStr.split(',')
    ,   anchors = [];
    for(var s=0; s<ankaArr.length; s++) {
      ...
      if(1 <= low && low <= 1001) { // ,で区切った安価が正常なレス番か
        ...
        for (var a=low; a<=high; a++) anchors.push(a);
      }
    }
  ...
  for(i = 0; i<loop; i++) {
    ...
    html += '<p id="res_' + (n+1) + '" style="margin-left:10px;">'; // レスのヘッダ（ブロック要素）
    html += '<span class="rh"><a href="#res_' +(n+1)+ '">' +(n+1)+ '</a></span> ：'; // レス番
    ...
    html += text.replace(/\n/g,"<br />")
                        ...
                        .replace(/>>([-0-9,\,]+)/g, '<a href="javascript:void(0);" class="anchor" data-anchor="$1">>>$1</a>')
                        ;
  }
  html += '<br />';
